...
func _ready() -> void:
	...
	await comp_text_display("AUTHORIZATION GRANTED.", tutorial_text, 0.05, 2.0, false)
	
	await comp_text_display("PROVIDING NECESSARY DATA", tutorial_text, 0.05, 0, true)
	...
	comp_text_display("", error_text_1, 0.07, 0, false)
	comp_text_display("", error_text_2, 0.07, 0, false)
	
	await get_tree().create_timer(1.5).timeout
	tutorial_text.text = ""
	
	await comp_text_display("WHAT MADE YOU THINK YOU WERE THE HUNTER?", tutorial_text, 0.2, 2.0, false)
	
	await comp_text_display("RULEBREAKERS, TRICKSTERS, AND DECEIVERS NEVER GO TRULY UNPUNISHED.", tutorial_text, 0.2, 2.0, false)
